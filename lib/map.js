...
export const numberCalc = (
  ...
  if (num1 && oper1 && num2 && oper2 && num3) {
    return eval(`${num1}${oper1}${num2}${oper2}${num3}`);
  } else if (num1 && oper1 && num2) {
    return eval(`${num1}${oper1}${num2}`);
  } else {
    return -99;
  }
};
...
const getAnswer = (items, level) => {
  switch (level) {
    ...
    case "medium":
      ...
      // items[4][5].value = numberCalc(items[4][0], items[4][1], items[4][2], items[4][3], items[4][4]);
      break;
    case "hard" || "crazy":
      items[0][5].value = numberCalc(
        items[0][0].value,
